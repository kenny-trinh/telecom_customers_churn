...
```{r DatasetOverview, echo=FALSE, results='show', cache=TRUE}
...
if (!file.exists("output/svm_tuned_model.rds")) {
# run with best parameters cost 10 and gamma 0.1 instead of tuning it again
tuned_parameters <- tune(svm, Churn ~ ., data = trainData,
                         ranges = list(cost = 10, 
                                       gamma = 0.1),
                         probability = TRUE)
  saveRDS(tuned_parameters$best.model, "output/svm_tuned_model.rds")
} else {
# Best model with probabilities
#best_model <- tuned_parameters$best.model
  best_model <- readRDS("output/svm_tuned_model.rds")
}
